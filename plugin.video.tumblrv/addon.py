...
def makeitem(name=None, img=None, path='blogposts', playable=False, **kwargs):
    ...
    try:
        if not path.startswith('plugin://'):
            ...
            itempath = path
        lbl2 = str(kwargs.get("label2", "") + Strip(kwargs.get('caption', itempath.partition('plugin.video.tumblrv/')[-1])))
        if img is None:
        ...
        try:
            if plugin.request.path.find('following/') != -1 or plugin.request.path.find('liked/') != -1 or plugin.request.path.find('dashboard/') != -1:
...
def index():
    ...
    try:
        ...
        lbldash = "[COLOR red]Tumblr Dashboard:[/COLOR] [COLOR white][B]{0}[/B][/COLOR] [I](Posts Since {1} {2})[/I]".format(tinfo.name, lastdashdate.date().isoformat(), lastdashdate.time().isoformat().rpartition(':')[0])
        lblfoldate = "Updated {0} {1}".format(lastfoldate.date().isoformat(), lastfoldate.time().isoformat().rpartition(':')[0])
        lblfol = "[COLOR orange]Following:[/COLOR] [COLOR white][B]{0}[/B][/COLOR] [I]({1})[/I]".format(str(tinfo.following), lblfoldate)
        lbllike = "[COLOR pink]Liked:[/COLOR] [COLOR white][B]{0}[/B][/COLOR]".format(str(tinfo.likes))
        itemdash = ListItem(label=lbldash, icon=dashimg, thumbnail=dashimg, path=plugin.url_for(dashboard, offset=0)) #, offset=0))
        itemfollowing = ListItem(label=lblfol, icon=__imgtumblr__, thumbnail=__imgtumblr__, path=plugin.url_for(blogs_following, offset=0))
...
#@plugin.route('/newdashboard/<offset>')
...
def dashboard(offset=0):
    ...
    try:
        ...
        lim = 20
        for off in range(startat, 0, (-1*lim)):
            items = dashboard_items(tclient.dashboard(limit=lim, offset=off, type='video', since_id=startid).get("posts", []))
            litems.extend(items)
        newestiditem = items[-1]
        if isinstance(newestiditem, ListItem):
            newestid = newestiditem.get_property(key='id')
        else:
            newestid = lbl2id(items[-1].label2)
        nextitem = ListItem(label="[B]Page #{0}[/B] ->".format(str(strpage)),
                            label2="{0}: {1} to {2}".format(str(nextoff), str(startid), str(newestid)),
                            icon=__imgnext__, thumbnail=__imgnext__, path=pathnext)
...
def dashboard_getitems(startoffset, max=60):
    ...
    postslist = []
    startid = plugin.get_setting('lastid', converter=int)
    startat = int(offset) + 60
    for offnum in range(startat, 0, -20):
        litems.extend(dashboard_items(tclient.dashboard(limit=20, offset=offnum, type='video', since_id=startid).get("posts", [])))
    if not doDebug():
        try:
...
def following_list(offset=0, max=0):
    ...
    try:
        for blog in blogs:
            ...
            li = ListItem(label=blogname, label2=updatetext, icon=thumb, thumbnail=thumb, path=plugin.url_for(endpoint=blogposts, blogname=blogname, offset=0))
            xitems.append(li)
    except Exception as ex:
        print str(repr(ex))
    try:
...
def blogs_following(offset=0):
    ...
    try:
        ...
        nextlbl = "[COLOR white]{1} to {2} of [I]{3}[/I][/COLOR]\n  [COLOR yellow]Next ->[/COLOR] [COLOR orange]PG#{0}[/COLOR]   ".format(str(strpage), str(nextoff), str(nextoffmax), str(totalfollow))
    ...
    for b in results:
        ...
        try:
            lbl = "[COLOR yellow][B]{0}[/B][/COLOR] [COLOR green][B]{4}/{5}[/B][/COLOR]\n[COLOR white][I]{1}[/I][/COLOR] ({2}:{3})".format(name, title.encode('latin-1', 'ignore'), updatetime.time.hour, updatetime.time.minute, updatetime.date.day, updatetime.datetime.month)
...
def blogposts(blogname, offset=0):
    ...
    if blogname is not None:
        ...
        for item in items:
            litems.append(makeitem(name=item.get('slug', blogname), img=item.get('thumbnail_url', 'DefaultVideo.png'), path=item.get('video_url'), playable=True, **item))
    else:
        results = tclient.posts(blogname=blogname, limit=20, offset=int(offset), type='video')
        if results is not None:
...
if __name__ == '__main__':
    ...
    if str(plugin.request.path).startswith('/taglist/') or str(plugin.request.path).find('/dashboard/') != -1  or plugin.request.path == '/': #or str(plugin.request.path).startswith('/blogsfollowing/')
        plugin.set_view_mode(viewmodel)
    else:
